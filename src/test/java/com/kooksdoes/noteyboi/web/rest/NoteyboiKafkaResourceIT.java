...
import com.kooksdoes.noteyboi.config.KafkaProperties;
...
class NoteyboiKafkaResourceIT {
    ...
    void setup() {
        ...
        NoteyboiKafkaResource kafkaResource = new NoteyboiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/noteyboi-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/noteyboi-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
